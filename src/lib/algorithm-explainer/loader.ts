...
// Auto-discover every JSON in ./algorithms/. Drop a new file there and it's picked up.
// TS infers JSON literal types per file, so cast via unknown to AlgorithmDef.
const modules = import.meta.glob("./algorithms/*.json", { eager: true });

export const ALGORITHMS: Record<string, AlgorithmDef> = {};
for (const mod of Object.values(modules)) {
    const def = (mod as { default: unknown }).default as AlgorithmDef;
    if (ALGORITHMS[def.id]) {
        throw new Error(`duplicate algorithm id: ${def.id}`);
    }
    validateAlgorithmDef(def);
    ALGORITHMS[def.id] = def;
}
